...
module.exports = {
  getRolePagedList: async ({ req, res }) => {
    ...
    const pagedList = await roleService.getRolePagedList(
      pageIndex,
      pageSize,
      sortBy,
      descending,
  ...
  delRoles: async ({ req, res }) => {
    const { ids, departmentIds } = req.body;
    await roleService
  ...
  savePermission: async ({ req, res }) => {
    const { moduleId, roleId, permissions } = req.body;
    // console.log('保存权限', permissions.length);
    const db = await roleService.savePermission(moduleId, roleId, permissions);
    if (db) {
