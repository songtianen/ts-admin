...
const buildDepartList = (deparmentList) => {
  ...
  return deparmentList.filter((item) => item.parentId === '1');
};
const buildRoleDepartTree = (deparmentList, roleArr) => {
  // eslint-disable-next-line no-unused-vars
  const departmentTree = buildDepartList(deparmentList);
...
// eslint-disable-next-line no-unused-vars
const copyMenu = (menuList) => JSON.parse(JSON.stringify(menuList));
// let c = [];
...
const getAllDepartmentAndRole = async (selector = {}) => {
  ...
  const deparmentList = copyMenu(deparment);
...
const editDepartment = async (data) => {
  if (data.id) {
    const db = await DepartmentModel.updateOne(
      { id: data.id },
